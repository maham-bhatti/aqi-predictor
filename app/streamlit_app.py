...
[data-testid="stMetricValue"]{font-family:'Space Grotesk',monospace !important;font-size:1.8rem !important;color:var(--accent) !important;font-weight:700 !important;}
...
.alert-danger{background:linear-gradient(90deg,#fff8f0,#fff3e8);border:1px solid #ffb74d;border-left:5px solid #f4511e;border-radius:12px;padding:0.9rem 1.4rem;margin:0.8rem 0;color:#bf360c;font-weight:600;font-size:0.9rem;}
.alert-warn{background:linear-gradient(90deg,#fffde7,#fff8e1);border:1px solid #ffe082;border-left:5px solid #ff9800;border-radius:12px;padding:0.9rem 1.4rem;margin:0.8rem 0;color:#e65100;font-weight:600;font-size:0.9rem;}
.alert-good{background:linear-gradient(90deg,#e8f5e9,#f1f8e9);border:1px solid #a5d6a7;border-left:5px solid #43a047;border-radius:12px;padding:0.9rem 1.4rem;margin:0.8rem 0;color:#1b5e20;font-weight:600;font-size:0.9rem;}
.forecast-card{background:var(--card-bg);border:1px solid var(--border);border-radius:16px;padding:1.3rem;text-align:center;backdrop-filter:blur(8px);box-shadow:0 4px 20px rgba(2,136,209,0.06);}
.section-title{font-family:'Space Grotesk',sans-serif;font-size:0.72rem;letter-spacing:0.14em;color:var(--accent);text-transform:uppercase;margin-bottom:0.5rem;border-bottom:1px solid var(--border);padding-bottom:0.45rem;font-weight:700;}
...
# ── Transparent background for all charts ────────────────────
...
def fetch_live_weather():
    try:
        ...
        return {"temperature":28,"humidity":55,"windspeed":8,"pressure":950,"precipitation":0,"weather_code":0,"hourly":{}}
...
def make_prediction(aqi_data, weather, models_tup, history_df):
    ...
    row = {
        ...
        "windspeed":weather["windspeed"],"pressure":weather["pressure"],"precipitation":weather["precipitation"],
        "hour":now.hour,"day_of_week":now.weekday(),"month":now.month,"is_weekend":int(now.weekday()>=5),
        "aqi_lag_1h":lag(1),"aqi_lag_3h":lag(3),"aqi_lag_6h":lag(6),"aqi_lag_24h":lag(24),"aqi_lag_48h":lag(48),
        "aqi_rolling_3h":roll(3),"aqi_rolling_6h":roll(6),"aqi_rolling_24h":roll(24),"aqi_rolling_48h":roll(48),
        "aqi_change_rate":aqi_data["aqi"]-lag(1),"aqi_change_6h":aqi_data["aqi"]-lag(6),"aqi_change_24h":aqi_data["aqi"]-lag(24),
        ...
        "is_winter":int(now.month in [11,12,1,2]),"is_summer":int(now.month in [5,6,7,8]),"is_monsoon":int(now.month in [7,8,9]),
    }
    try:
...
<div style='display:flex;align-items:center;justify-content:space-between;
    ...
    <div style='background:linear-gradient(135deg,#e3f2fd,#e0f7fa);color:#0277bd;font-size:0.78rem;
                font-weight:700;padding:0.45rem 1rem;border-radius:20px;border:1px solid rgba(2,136,209,0.25);'>
        📡 Live Data
    </div>
</div>
""", unsafe_allow_html=True)
...
with col_gauge:
    ...
    fig_g = go.Figure(go.Indicator(
        ...
        title={"text":f"{emoji} {cat}","font":{"size":17,"color":"#1a3a4a","family":"Space Grotesk"}},
        gauge={
            "axis":{"range":[0,300],"tickcolor":"#5a8fa8","tickfont":{"color":"#5a8fa8","size":10}},
            ...
            "threshold":{"line":{"color":"#1a3a4a","width":3},"thickness":0.8,"value":aqi_data["aqi"]},
        },
    ...
    st.markdown(f"<div style='text-align:center;color:#5a8fa8;font-size:0.75rem;font-family:Space Grotesk;margin-top:-0.5rem;'>Last updated: {now_str}</div>", unsafe_allow_html=True)
...
with col_weather:
    ...
    fig_p.update_layout(
        ...
        plot_bgcolor=TRANSPARENT,
        showlegend=False,
        xaxis=dict(showgrid=False, color="#5a8fa8", tickfont=dict(size=11, color="#5a8fa8")),
        yaxis=dict(showgrid=True, gridcolor="rgba(2,136,209,0.1)", color="#5a8fa8",
                   title="μg/m³", titlefont=dict(color="#5a8fa8")),
    )
...
for col, (label, pred, tag) in zip([fc1,fc2,fc3],
    ...
    with col:
        ...
        <div class='forecast-card' style='border-top:4px solid {fcol};'>
            <p style='color:#5a8fa8;font-size:0.72rem;font-family:Space Grotesk;letter-spacing:0.1em;font-weight:700;margin:0;'>{tag.upper()} FORECAST</p>
            <h3 style='color:{fcol};font-size:2.8rem;margin:0.3rem 0;font-family:Space Grotesk;font-weight:900;'>{pred:.0f}</h3>
            <p style='color:#5a8fa8;font-size:0.8rem;margin:0;'>AQI · {label}</p>
            <p style='font-size:1.6rem;margin:0.5rem 0;'>{femo}</p>
            <span style='background:{fcol}18;color:{fcol};padding:0.25rem 0.9rem;border-radius:20px;font-size:0.73rem;font-weight:700;font-family:Space Grotesk;border:1px solid {fcol}35;'>{fcat}</span>
            <p style='color:#5a8fa8;font-size:0.72rem;margin:0.6rem 0 0;line-height:1.4;'>{aqi_advice(pred)[:65]}...</p>
...
with col_hist:
    ...
    if not history_df.empty:
        ...
        fig_h.update_layout(
            ...
            plot_bgcolor=TRANSPARENT,
            hovermode="x unified",
            xaxis=dict(showgrid=False, color="#5a8fa8", tickfont=dict(color="#5a8fa8")),
            yaxis=dict(showgrid=True, gridcolor="rgba(2,136,209,0.1)", color="#5a8fa8",
                       title="AQI", titlefont=dict(color="#5a8fa8")),
        )
    ...
    if hourly:
        ...
        fig_hw.update_layout(
            ...
            xaxis=dict(showgrid=False, color="#5a8fa8",
                       ...
                       tickfont=dict(color="#5a8fa8")),
            yaxis=dict(showgrid=True, gridcolor="rgba(2,136,209,0.1)",
                       color="#f4511e", title="Temperature °C", titlefont=dict(color="#f4511e")),
            yaxis2=dict(overlaying="y", side="right", color="#0288d1",
                        title="Rain Prob %", titlefont=dict(color="#0288d1"), range=[0,100]),
            legend=dict(bgcolor="rgba(0,0,0,0)", orientation="h", y=1.1,
                        font=dict(color="#5a8fa8")),
        )
...
with col_heat:
    ...
    if not history_df.empty and len(history_df) > 7:
        ...
        fig_heat = go.Figure(go.Heatmap(
            ...
            colorbar=dict(title="AQI", tickfont=dict(color="#5a8fa8"), title_font=dict(color="#5a8fa8")),
            hoverongaps=False,
        ))
        fig_heat.update_layout(
            ...
            paper_bgcolor=TRANSPARENT,
            plot_bgcolor=TRANSPARENT,
            xaxis=dict(showgrid=False, color="#5a8fa8", title="Date",
                       nticks=10, tickfont=dict(color="#5a8fa8")),
            yaxis=dict(showgrid=False, color="#5a8fa8", title="Hour of Day",
                       titlefont=dict(color="#5a8fa8"), autorange="reversed"),
        )
...
with col_metrics:
    ...
    if met:
        ...
        <div style='background:rgba(2,136,209,0.07);border:1px solid rgba(2,136,209,0.25);
            ...
            <p style='color:#5a8fa8;font-size:0.7rem;font-family:Space Grotesk;margin:0;letter-spacing:0.1em;font-weight:700;'>BEST MODEL</p>
            <p style='color:#0288d1;font-size:1.05rem;font-family:Space Grotesk;margin:0.2rem 0 0;font-weight:800;'>{bm}</p>
        </div>""", unsafe_allow_html=True)
        for horizon, key in [("24h Forecast","model_24h"),("48h Forecast","model_48h"),("72h Forecast","model_72h")]:
            ...
            if m:
                ...
                st.markdown(f"""
                <div style='background:rgba(240,247,255,0.8);border:1px solid rgba(2,136,209,0.15);
                            border-radius:10px;padding:0.7rem 1rem;margin:0.4rem 0;'>
                    <span style='color:#5a8fa8;font-size:0.78rem;font-family:Space Grotesk;font-weight:600;'>{horizon}</span>
                    <span style='float:right;'>
                        <span style='color:{r2c};font-family:Space Grotesk;font-size:0.82rem;font-weight:800;'>R²={r2:.3f}</span>
                        <span style='color:#5a8fa8;font-size:0.73rem;margin-left:0.5rem;'>RMSE={rmse:.1f}</span>
                    </span>
                </div>""", unsafe_allow_html=True)
        n_feat = met.get("n_features", 0)
        train  = met.get("train_size",  0)
        st.markdown(f"<div style='margin-top:0.8rem;color:#5a8fa8;font-size:0.78rem;line-height:1.5;'>📊 Trained on <b style='color:#0288d1;'>{train:,}</b> hourly records using <b style='color:#0288d1;'>{n_feat}</b> features</div>", unsafe_allow_html=True)
    else:
...
for col, (rng,cat,clr,ico,desc) in zip(scale_cols,[
    ...
    with col:
        ...
        <div style='background:{clr}10;border:1px solid {clr}35;border-top:3px solid {clr};
                    border-radius:11px;padding:0.7rem 0.4rem;text-align:center;box-shadow:0 2px 10px {clr}12;'>
            <p style='font-size:1.3rem;margin:0;'>{ico}</p>
            <p style='color:{clr};font-family:Space Grotesk;font-size:0.73rem;font-weight:800;margin:0.25rem 0;'>{rng}</p>
            <p style='color:#1a3a4a;font-size:0.7rem;margin:0;font-weight:500;'>{cat}</p>
            <p style='color:#5a8fa8;font-size:0.63rem;margin:0.2rem 0 0;'>{desc}</p>
        </div>""", unsafe_allow_html=True)

st.markdown("<br>", unsafe_allow_html=True)
st.markdown("""
<div style='text-align:center;color:#5a8fa8;font-size:0.73rem;font-family:Space Grotesk;
