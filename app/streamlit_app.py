...
div[data-testid="stHorizontalBlock"] > div {
    ...
    box-shadow: 0 4px 20px rgba(2,136,209,0.06);
}
.alert-danger {
...
# ── Helper: transparent plotly layout ────────────────────────
def plot_base(height, margin=None):
    """Returns common layout kwargs — no font_color (removed for Plotly compat)."""
    if margin is None:
...
<div style='display:flex; align-items:center; justify-content:space-between;
            padding:0.8rem 0 1rem; border-bottom:2px solid rgba(2,136,209,0.15);
            margin-bottom:1rem;'>
    <div>
    ...
    <div style='background:linear-gradient(135deg,#e3f2fd,#e0f7fa);
                color:#0277bd; font-size:0.78rem; font-weight:700;
...
if aqi_val > 200:
    ...
    st.markdown(f"<div class='alert-warn'>⚠️ <b>CAUTION</b> — AQI is <b>{aqi_val:.0f}</b> (Moderate–Unhealthy). Sensitive individuals should limit outdoor exposure.</div>", unsafe_allow_html=True)
else:
...
with col_gauge:
    ...
    fig_g = go.Figure(go.Indicator(
        ...
        title={"text": f"{emoji} {cat}",
               "font": {"size": 17, "color": "#1a3a4a", "family": "Space Grotesk"}},
        gauge={
            "axis": {"range": [0, 300], "tickcolor": "#5a8fa8",
                     "tickfont": {"color": "#5a8fa8", "size": 10}},
            ...
            "threshold": {"line": {"color": "#1a3a4a", "width": 3},
    ...
    fig_g.update_layout(
        **plot_base(300, margin=dict(l=20, r=20, t=40, b=20))
    )
    st.plotly_chart(fig_g, use_container_width=True)
    st.markdown(f"""
    <div style='text-align:center; color:#5a8fa8; font-size:0.75rem;
                font-family:Space Grotesk; margin-top:-0.5rem;'>
...
with col_weather:
    ...
    fig_p.update_layout(
        **plot_base(220, margin=dict(l=0, r=0, t=20, b=0)),
...
for col, (label, pred, tag) in zip([fc1, fc2, fc3],
    ...
    with col:
        ...
        <div class='forecast-card' style='border-top:4px solid {fcol};'>
            <p style='color:#5a8fa8; font-size:0.72rem; font-family:Space Grotesk;
            ...
            <span style='background:{fcol}18; color:{fcol}; padding:0.25rem 0.9rem;
                         ...
                         font-family:Space Grotesk; border:1px solid {fcol}35;'>
                {fcat}
            </span>
            <p style='color:#5a8fa8; font-size:0.72rem; margin:0.6rem 0 0;
        ...
        fig_hw.update_layout(
            **plot_base(280),
            xaxis=dict(showgrid=False, color="#5a8fa8",
                       ...
                       tickfont=dict(color="#5a8fa8")),
            yaxis=dict(showgrid=True, gridcolor="rgba(2,136,209,0.1)",
                       color="#f4511e", title="Temperature °C",
                       titlefont=dict(color="#f4511e")),
            yaxis2=dict(overlaying="y", side="right",
                        color="#0288d1", title="Rain Prob %",
                        titlefont=dict(color="#0288d1"), range=[0, 100]),
...
with col_metrics:
    ...
    if met:
        ...
        <div style='background:rgba(2,136,209,0.07); border:1px solid rgba(2,136,209,0.25);
            ...
            <p style='color:#5a8fa8; font-size:0.7rem; font-family:Space Grotesk;
        ...
        for horizon, key in [("24h Forecast","model_24h"),
            ...
            if m:
                ...
                <div style='background:rgba(240,247,255,0.8);
                    ...
                    <span style='color:#5a8fa8; font-size:0.78rem;
                    ...
                    <span style='float:right;'>
                        ...
                        <span style='color:#5a8fa8; font-size:0.73rem;
        ...
        st.markdown(f"""
        <div style='margin-top:0.8rem; color:#5a8fa8; font-size:0.78rem; line-height:1.5;'>
...
st.markdown("<br>", unsafe_allow_html=True)
st.markdown("""
<div style='text-align:center; color:#5a8fa8; font-size:0.73rem;
